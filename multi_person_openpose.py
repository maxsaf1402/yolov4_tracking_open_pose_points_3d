...
def open_pose(frame, color):
    ...
    for i in range(nPoints):
        for j in range(len(detected_keypoints[i])):
            cv2.circle(frameClone, detected_keypoints[i][j][0:2], 5, color, -1, cv2.LINE_AA)
    ...
    for i in range(17):
        for n in range(len(personwiseKeypoints)):
            ...
            cv2.line(frameClone, (B[0], A[0]), (B[1], A[1]), color, 3)
    return frameClone, detected_keypoints
